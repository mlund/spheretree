...
GLint gl2psListNbr(GL2PSlist *list) { return (list->n); }
...
GLvoid gl2psAddIndex(GLshort *index0, GLshort *index1, GLshort *nb, GLshort i,
                     GLshort j) {
  GLint k;
...
GLvoid gl2psDivideQuad(GL2PSprimitive *quad, GL2PSprimitive **t1,
                       GL2PSprimitive **t2) {
...
GLvoid gl2psAddInImageTree(GL2PSprimitive *prim, GL2PSxyz a, GL2PSxyz b,
                           GL2PSbsptree2d **tree) {
  GLint res;
  ...
  if (*tree == NULL) {
    ...
    case GL2PS_IN_FRONT_OF:
      if ((*tree)->flag)
        gl2psAddInImageTree(prim, a, b, &(*tree)->front);
      break;
    case GL2PS_SPANNING:
      gl2psAddInImageTree(prim, a, b, &(*tree)->back);
      if ((*tree)->flag)
  ...
  if (prim->numverts < 3)
    ...
    for (i = 0; i < prim->numverts; i++) {
      ...
      gl2psAddInImageTree(prim, prim->verts[i].xyz,
                          prim->verts[gl2psGetIndex(i, prim->numverts)].xyz,
                          &image);
    }
  }
...
GLvoid gl2psAddPolyPrimitive(GLshort type, GLshort numverts, GL2PSvertex *verts,
                             GLint offset, GLshort dash, GLshort width,
                             GLshort boundary) {
  GLshort i;
...
GLint gl2psParseFeedbackBuffer(GLvoid) {
  ...
  while (used > 0) {
      ...
      while (count > 0 && used > 0) {
        ...
        if (v == 2) {
          if (boundary) {
            ...
            flag = 0;
          gl2psAddPolyPrimitive(GL2PS_TRIANGLE, 3, vertices, offset, dash, 1,
                                flag);
    ...
    case GL_PASS_THROUGH_TOKEN:
      ...
      case GL2PS_BEGIN_POLYGON_OFFSET_FILL:
        offset = 1;
        break;
      case GL2PS_END_POLYGON_OFFSET_FILL:
        offset = 0;
        break;
      case GL2PS_BEGIN_POLYGON_BOUNDARY:
        boundary = 1;
        break;
      case GL2PS_END_POLYGON_BOUNDARY:
        boundary = 0;
        break;
      case GL2PS_BEGIN_LINE_STIPPLE:
        dash = 4;
        break;
      case GL2PS_END_LINE_STIPPLE:
        dash = 0;
        break;
...
GLvoid gl2psPrintPostscriptHeader(GLvoid) {
  ...
  */
  ...
  fprintf(
      ...
      "      /x2 exch def b2 b1 sub abs 0.01 gt g2 g1 sub abs 0.005 gt r2 r1 "
      "sub\n"
      "      abs 0.008 gt or or { /bm b1 b2 add 0.5 mul def /gm g1 g2 add 0.5 "
      "mul def\n"
      "      /rm r1 r2 add 0.5 mul def /ym y1 y2 add 0.5 mul def /xm x1 x2 "
      "add\n"
      "      0.5 mul def x1 y1 r1 g1 b1 xm ym rm gm bm SL xm ym rm gm bm x2 y2 "
      "r2\n"
      ...
      "      b2 b1 sub abs 0.05 gt g2 g1 sub abs 0.017 gt r2 r1 sub abs 0.032 "
      "gt\n"
      "      b3 b1 sub abs 0.05 gt g3 g1 sub abs 0.017 gt r3 r1 sub abs 0.032 "
      "gt\n"
      "      b2 b3 sub abs 0.05 gt g2 g3 sub abs 0.017 gt r2 r3 sub abs 0.032 "
      "gt\n"
      "      or or or or or or or or { /b12 b1 b2 add 0.5 mul def /g12 g1 g2 "
      "add\n"
      "      0.5 mul def /r12 r1 r2 add 0.5 mul def /y12 y1 y2 add 0.5 mul "
      "def\n"
      "      /x12 x1 x2 add 0.5 mul def /b13 b1 b3 add 0.5 mul def /g13 g1 g3\n"
      "      add 0.5 mul def /r13 r1 r3 add 0.5 mul def /y13 y1 y3 add 0.5 "
      "mul\n"
      "      def /x13 x1 x3 add 0.5 mul def /b32 b3 b2 add 0.5 mul def\n"
      "      /g32 g3 g2 add 0.5 mul def /r32 r3 r2 add 0.5 mul def /y32 y3 y2\n"
      "      add 0.5 mul def /x32 x3 x2 add 0.5 mul def x1 y1 r1 g1 b1 x12 "
      "y12\n"
      "      r12 g12 b12 x13 y13 r13 g13 b13 x2 y2 r2 g2 b2 x12 y12 r12 g12 "
      "b12\n"
      "      x32 y32 r32 g32 b32 x3 y3 r3 g3 b3 x32 y32 r32 g32 b32 x13 y13 "
      "r13\n"
      "      g13 b13 x32 y32 r32 g32 b32 x12 y12 r12 g12 b12 x13 y13 r13 g13 "
      "b13\n"
      ...
      "gsave\n"
      "1.0 1.0 scale\n",
      gl2ps.title, GL2PS_VERSION, gl2ps.producer, ctime(&now), viewport[2],
      viewport[3], viewport[0], viewport[1], viewport[2], viewport[3]);
  ...
  if (gl2ps.options & GL2PS_DRAW_BACKGROUND) {
    ...
    fprintf(gl2ps.stream,
            ...
            "closepath fill\n",
            rgba[0], rgba[1], rgba[2], viewport[0], viewport[1], viewport[2],
            viewport[1], viewport[2], viewport[3], viewport[0], viewport[3]);
  }
}
...
GLvoid gl2psPrintPostscriptPrimitive(GLvoid *a, GLvoid *b) {
  ...
  if (gl2ps.options & GL2PS_OCCLUSION_CULL && prim->depth >= 0.)
    return;
  ...
  case GL2PS_TEXT:
    ...
    fprintf(gl2ps.stream, "(%s) %g %g %d /%s S\n", prim->text->str,
            prim->verts[0].xyz[0], prim->verts[0].xyz[1], prim->text->fontsize,
            prim->text->fontname);
    break;
  case GL2PS_POINT:
    PRINTCOLOR;
  ...
  case GL2PS_TRIANGLE:
    if (gl2ps.shade) {
      fprintf(
          gl2ps.stream, "%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g ST\n",
...
GLvoid gl2psBeginPage(char *title, char *producer, GLint sort, GLint options,
  ...
  gl2ps.format =
      GL2PS_EPS; /* a new arg should be introduced to select the format */
  gl2ps.title = title;
...
GLint gl2psEndPage(GLvoid) {
  ...
  if (res == GL2PS_SUCCESS) {
    ...
    case GL2PS_BSP_SORT:
      ...
      gl2psTraverseBspTree(root, eye, GL2PS_EPSILON, gl2psGreater, pprim);
