...
realT qh_determinant(realT **rows, int dim, boolT *nearzero) {
  ...
  *nearzero = False;
  if (dim < 2) {
    fprintf(qh ferr, "qhull internal error (qh_determinate): only implemented "
                     "for dimension >= 2\n");
...
-facetarea- return area for a facet
  if non-simplicial, uses centrum to triangulate facet.  Sums the projected
areas. assumes facet->normal exists if (qh DELAUNAY), computes projected area
instead for last coordinate
*/
...
REALepsilon %g REALmin %g REALmax %g -REALmax %g\n",
  ...
  for (k = 0; k < dimension; k++) {
    ...
    if (qh SCALElast && k == dimension - 1)
      qh_scalelast(points, numpoints, dimension, minimum[k], maximum[k],
                   qh MAXlowcoord);
...
void qh_maxsimplex(int dim, setT *maxpoints, pointT *points, int numpoints,
  ...
  for (k = sizinit; k < dim + 1; k++) {
    ...
    if (!maxpoint) {
      fprintf(qh ferr, "qhull internal error (qh_maxsimplex): not enough "
                       "points available\n");
      qh_errexit(qh_ERRqhull, NULL, NULL);
    }
...
*/
void qh_printmatrix(FILE *fp, char *string, realT **rows, int numrow,
                    int numcol) {
  realT *rowp;
  realT r; /*bug fix*/
  int i, k;
...
/*-------------------------------------------------
-projectinput- project input points using qh DELAUNAY and qh
lower_bound/upper_bound input points in qh first_point, num_points, input_dim if
POINTSmalloc, will free old point array if low[k]=high[k]= 0, removes dimension
k checks that hull_dim agrees with input_dim, PROJECTinput, and DELAUNAY if
DELAUNAY projects points to paraboloid if qh ATinfinity, adds point
"at-infinity" returns: new point array in first_point of qh hull_dim coordinates
  sets POINTSmalloc
...
void qh_projectpoints(signed char *project, int n, realT *points, int numpoints,
  ...
  if (testdim != newdim) {
    fprintf(qh ferr,
            "qhull internal error (qh_projectpoints): newdim %d should be %d "
            "after projection\n",
            newdim, testdim);
    qh_errexit(qh_ERRqhull, NULL, NULL);
  }
...
void qh_scalelast(coordT *points, int numpoints, int dim, coordT low,
  ...
  if (nearzero) {
    fprintf(qh ferr,
            "qhull input error: last coordinate's new bounds [0, %2.2g] too "
...
void qh_scalepoints(pointT *points, int numpoints, int dim, realT *newlows,
  ...
  for (k = 0; k < dim; k++) {
    ...
    if (nearzero) {
      fprintf(qh ferr,
              "qhull input error: %d'th dimension's new bounds [%2.2g, %2.2g] "
              "too wide for\nexisting bounds [%2.2g, %2.2g]\n",
...
LABELerroroutside:
  feasiblep = feasible;
  normp = normal;
  fprintf(qh ferr, "qhull input error: feasible point is not clearly inside "
                   "halfspace\nfeasible point: ");
  for (k = dim; k--;)
...
#ifndef qh_NOtrace
    if (qh IStracing >= 3) {
      fprintf(qh ferr, "qh_voronoi_center: det %2.2g factor %2.2g ", det,
              factor);
