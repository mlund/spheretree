...
/*----------- internal macros -------------------
-SETsizeaddr_(set) - return pointer to actual size+1 of set (set CANNOT be
NULL!!) *SETsizeaddr==NULL or e[*SETsizeaddr-1].p==NULL
*/
...
void qh_setcheck(setT *set, char *tname, int id) {
  ...
  if (size > maxsize || !maxsize) {
    ...
    fprintf(qhmem.ferr,
            "qhull internal error (qh_setcheck): %s%d (size %d max %d) is not "
            "null terminated.\n",
...
void qh_setprint(FILE *fp, char *string, setT *set) {
  ...
  if (!set)
    ...
    SETreturnsize_(set, size);
    fprintf(fp, "%s set=%p maxsize=%d size=%d elems=", string, set,
            set->maxsize, size);
    if (size > set->maxsize)
      size = set->maxsize + 1;
    for (k = 0; k < size; k++)
...
/*----------------------------------------
-settemppop- pop and return temporary set from qhmem.tempstack (makes it
permanent)
*/
setT *qh_settemppop(void) {
  ...
  if (!stackedset) {
    fprintf(qhmem.ferr, "qhull internal error (qh_settemppop): pop from empty "
                        "temporary stack\n");
    qh_errexit(qhmem_ERRqhull, NULL, NULL);
  }
  if (qhmem.IStracing >= 5)
