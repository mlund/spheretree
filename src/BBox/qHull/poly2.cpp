...
void qh_check_maxout(void) {
  ...
  trace1((qh ferr,
          "qh_check_maxout: determine actual maxoutside and minoutside\n"));
  ...
  trace1((qh ferr,
          "qh_check_maxout: maxoutside %2.2g, minvertex %2.2g, outside of not "
          "good %d\n",
...
void qh_check_points(void) {
  ...
  maxoutside =
      fmax_(qh max_outside,
            qh DISTround); /* agrees with qh_printafacet, qh_printsummary */
  maxoutside += 2 * qh DISTround;
  /* 1 DISTround to actual point and another DISTround to computed point */
  if (qh RANDOMdist) /* repeated computations can differ by 2*distround */
    maxoutside += qh DISTround;
    ...
    if (!allsimplicial) {
      ...
      FOREACHneighbor_(facet) {
        ...
        if (dist > qh DISTround) {
          zzinc_(Zconcaveridges);
          fprintf(qh ferr,
                  "qhull precision error: f%d is concave to f%d.  Centrum of "
                  "f%d is %6.4g above f%d\n",
...
void qh_checkfacet(facetT *facet, boolT newmerge, boolT *waserrorp) {
  ...
  FOREACHvertex_(facet->vertices) {
    ...
    if (vertex->id >= previousid) {
      fprintf(qh ferr,
              "qhull internal error (qh_checkfacet): vertices of f%d are not "
              "in descending id order at v%d\n",
              facet->id, vertex->id);
      waserror = True;
      break;
    }
  ...
  if (facet->simplicial) {
    ...
    if (!newmerge &&
      ...
      fprintf(qh ferr,
              "qhull internal error (qh_checkfacet): for facet f%d, #vertices "
              "%d or #neighbors %d < qh hull_dim\n",
              facet->id, numvertices, numneighbors);
      waserror = True;
    }
  ...
  FOREACHridge_(facet->ridges) {
    ...
    if (numRvertices != qh hull_dim - 1) {
      fprintf(qh ferr,
              "qhull internal error (qh_checkfacet): ridge between f%d and f%d "
              "has %d vertices\n",
    ...
    if (!qh_setin(facet->neighbors, neighbor)) {
      fprintf(qh ferr,
              "qhull internal error (qh_checkfacet): for facet f%d, neighbor "
              "f%d of ridge r%d not in facet\n",
    ...
    FOREACHneighbor_(facet) {
      ...
      FOREACHridge_(facet->ridges) {
        ...
        FOREACHvertex_(ridge->vertices) {
          if (!vertex->seen) {
            fprintf(qh ferr,
                    "qhull internal error (qh_checkfacet): vertex v%d in r%d "
                    "not in f%d intersect f%d\n",
...
void qh_checkpolygon(facetT *facetlist) {
  ...
  if (facetlist == qh facet_list) {
    if (numfacets != qh num_facets - qh num_visible) {
      fprintf(qh ferr,
              "qhull internal error (qh_checkpolygon): actual number of facets "
              "is %d, cumulative facet count is %d\n",
  ...
  if (qh GOODvertex < 0 || (qh GOODvertex > 0 && qh MERGING)) {
    FORALLfacet_(facetlist) {
      if (facet->good && ((qh GOODvertex > 0) ^
        ...
        if (!--numgood) {
          fprintf(qh ferr,
                  "qhull warning: good vertex p%d does not match last good "
                  "facet f%d.  Ignored.\n",
...
void qh_infiniteloop(facetT *facet) {
  ...
  fprintf(qh ferr, "qhull internal error (qh_infiniteloop): potential infinite "
                   "loop detected\n");
  qh_errexit(qh_ERRqhull, facet, NULL);
} /* qh_infiniteloop */
...
void qh_initialhull(setT *vertices) {
  ...
  FORALLfacets {
    if (!qh_checkflipped(facet, NULL,
      ...
      trace1((qh ferr, "qh_initialhull: initial orientation incorrect.  "
                       "Correct all facets\n"));
      facet->flipped = False;
      FORALLfacets {
  ...
  FORALLfacets {
    if (!qh_checkflipped(facet, NULL, !qh_ALL)) { /* can happen with 'R0.1' */
      fprintf(qh ferr,
              "qhull precision error: facet %d is coplanar with the interior "
              "point\n",
              facet->id);
...
notes:
  ...
  if dim >= qh_INITIALmax, uses min/max x and max points with non-zero
determinants
*/
...
void qh_matchduplicates(facetT *atfacet, int atskip, int hashsize,
  ...
  for (makematch = 0; makematch < 2; makematch++) {
      ...
      for (scan = hash; (facet = SETelemt_(qh hash_table, scan, facetT));
        ...
        if (qh_matchvertices(1, newfacet->vertices, newskip, facet->vertices,
          ...
          if (SETelemt_(facet->neighbors, skip, facetT) != qh_DUPLICATEridge) {
            if (!makematch) {
              fprintf(qh ferr,
                      "qhull internal error (qh_matchduplicates): missing "
                      "dupridge at f%d skip %d for new f%d skip %d hash %d\n",
    ...
    if (!makematch) {
      if (!maxmatch) {
        fprintf(qh ferr,
                "qhull internal error (qh_matchduplicates): no maximum match "
                "at duplicate f%d skip %d at hash %d\n",
...
void qh_printlists(void) {
  ...
  FORALLfacets fprintf(qh ferr, " %d", facet->id);
  fprintf(qh ferr,
  ...
  FORALLvertices fprintf(qh ferr, " %d", vertex->id);
  fprintf(qh ferr, "\n");
} /* printlists */
