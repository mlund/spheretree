  ...
  for (scan = hash; (facet = SETelemt_(qh hash_table, scan, facetT));
    ...
    if (qh_matchvertices(1, newfacet->vertices, newskip, facet->vertices, &skip,
      ...
      if (SETelem_(newfacet->vertices, newskip) ==
        ...
        fprintf(qh ferr,
                "qhull precision error: Vertex sets are the same for f%d and "
                "f%d.  Can not force output.\n",
...
void qh_updatevertices(void) {
  ...
  trace3((qh ferr, "qh_updatevertices: delete interior vertices and update "
                   "vertex->neighbors\n"));
