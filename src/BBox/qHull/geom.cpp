...
returns:
     normal= x
     if can't divzero() for later normalization (qh MINdenom_2 and qh
MINdenom_1_2), sets tail of normal to [...,sign,0,...], i.e., solves for b=
[0...] sets nearzero, unless last row (i.e., hyperplane intersects [0,..,1])
notes:
...
void qh_distplane(pointT *point, facetT *facet, realT *dist) {
  ...
  if (qh RANDOMdist) {
    randr = qh_RANDOMint;
    *dist +=
        (2.0 * randr / qh_RANDOMmax - 1.0) * qh RANDOMfactor * qh maxmaxcoord;
  }
  if (qh IStracing >= 4) {
...
facetT *qh_findbest(pointT *point, facetT *facet, boolT bestoutside,
  ...
  if (!ischeckmax && !ispartition && !isfindfacet) {
    fprintf(qh ferr, "qhull internal error (qh_findbest): unknown combination "
                     "of arguments\n");
...
#if qh_MAXoutside
    if (ischeckmax && (!qh ONLYgood || facet->good) &&
...
void qh_setfacetplane(facetT *facet) {
  ...
  if (qh IStracing >= 3) {
    fprintf(qh ferr, "qh_setfacetplane: f%d offset %2.2g normal: ", facet->id,
            facet->offset);
    for (k = 0; k < qh hull_dim; k++)
