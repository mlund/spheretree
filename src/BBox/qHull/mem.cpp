...
void *qh_memalloc(int insize) {
  ...
  if ((unsigned)insize <= (unsigned)qhmem.LASTsize) {
    ...
    if (!qhmem.indextable) {
      fprintf(qhmem.ferr, "qhull internal error (qh_memalloc): qhmem has not "
                          "been initialized.\n");
...
void qh_meminit(FILE *ferr) {
  ...
  if (sizeof(void *) < sizeof(int)) {
    fprintf(ferr, "qhull internal error (qh_meminit): sizeof(void*) < "
                  "sizeof(int).  set.c will not work\n");
...
void qh_meminitbuffers(int tracelevel, int alignment, int numsizes, int bufsize,
  ...
  if (qhmem.ALIGNmask & ~qhmem.ALIGNmask) {
    fprintf(qhmem.ferr,
            "qhull internal error (qh_meminit): memory alignment %d is not a "
            "power of 2\n",
            alignment);
    qh_errexit(qhmem_ERRqhull, NULL, NULL);
  }
...
void qh_memsetup(void) {
  ...
  if (qhmem.LASTsize >= qhmem.BUFsize || qhmem.LASTsize >= qhmem.BUFinit) {
    fprintf(qhmem.ferr,
            "qhull error (qh_memsetup): largest mem size %d is >= buffer size "
            "%d or initial buffer size %d\n",
