/* merge.c - merges non-convex facets
   ...
   the user may call qh_postmerge() to perform additional merges.
     ...
     To remove deleted facets and vertices (qhull() in qhull.c):
      qh_partitionvisible (!qh_ALL, &numoutside);  // visible_list,
   newfacet_list qh_deletevisible ();         // qh visible_list qh_resetlists
   (False);       // qh visible_list newvertex_list newfacet_list
...
void qh_all_merges(boolT othermerge, boolT vneighbors) {
  ...
  trace2((qh ferr,
          "qh_merge_all: starting to merge facets beginning from f%d\n",
  ...
  trace1((qh ferr,
          "qh_merge_all: merged %d coplanar facets %d concave facets and %d "
...
/*------------------------------------------------
-copynonconvex- set non-convex flag on another ridge (if any) between same
neighbors may be faster if use smaller ridge set
*/
...
/*------------------------------------------------
-degen_redundant_neighbors- append degen. and redundant neighbors to
facet_mergeset also checks current facet for degeneracy bumps vertex_visit
...
vertexT *qh_find_newvertex(vertexT *oldvertex, setT *vertices, setT *ridges) {
  ...
  if (qh IStracing >= 4) {
    fprintf(qh ferr, "qh_find_newvertex: find new vertex for v%d from ",
            oldvertex->id);
    FOREACHvertex_(vertices) fprintf(qh ferr, "v%d ", vertex->id);
    FOREACHridge_(ridges) fprintf(qh ferr, "r%d ", ridge->id);
    fprintf(qh ferr, "\n");
  }
#endif
...
/*-------------------------------------------
-maydropneighbor -- drop neighbor relationship if no ridge between facet and
neighbor bumps qh visit_id returns: appends degenerate facets to facet_mergeset
...
void qh_mergecycle(facetT *samecycle, facetT *newfacet) {
  ...
  if (qh IStracing >= 4) {
    ...
    FORALLsame_cycle_(samecycle) fprintf(qh ferr, " f%d", same->id);
    fprintf(qh ferr, "\n");
  }
  if (qh IStracing >= 4)
...
void qh_mergefacet(facetT *facet1, facetT *facet2, realT *mindist,
...
#ifndef qh_NOtrace
  if (mindist &&
      (-*mindist > qh OUTPUTINFOdist || *maxdist > qh OUTPUTINFOdist)) {
    tracerestore = 0;
    ...
    traceonce = True;
    fprintf(qh ferr,
            "qh_mergefacet: ========= trace merge #%d involving f%d, furthest "
            "is p%d\n",
  ...
  if (facet1 == facet2 || facet1->visible || facet2->visible) {
    fprintf(qh ferr,
            "qhull internal error (qh_mergefacet): either f%d and f%d are the "
            "same or one is a visible facet\n",
            facet1->id, facet2->id);
    qh_errexit2(qh_ERRqhull, facet1, facet2);
  }
...
void qh_mergevertices(setT *vertices1, setT **vertices2) {
  ...
  if (newsize < qh_setsize(mergedvertices)) {
    fprintf(qh ferr, "qhull internal error (qh_mergevertices): facets did not "
                     "share a ridge\n");
...
vertexT *qh_rename_sharedvertex(vertexT *vertex, facetT *facet) {
  ...
  if (qh_setsize(vertex->neighbors) == 2) {
    ...
    if (!neighborA) {
      fprintf(qh ferr,
              "qhull internal error (qh_rename_sharedvertex): v%d's neighbors "
              "not in f%d\n",
              vertex->id, facet->id);
