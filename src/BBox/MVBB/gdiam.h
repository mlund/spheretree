...
template <class T> inline void gdiam_exchange(T &a, T &b) {
  T tmp = a;
...
inline void pnt_zero(gdiam_point dst) { dst[0] = dst[1] = dst[2] = 0; }
inline void pnt_dump(gdiam_point_cnt pnt) {
  printf("(%g, %g, %g)\n", pnt[0], pnt[1], pnt[2]);
}

inline void pnt_copy(gdiam_point dst, const gdiam_point src) {
  dst[0] = src[0];
  dst[1] = src[1];
  dst[2] = src[2];
}
...
inline void pnt_init(gdiam_point pnt, gdiam_real x, gdiam_real y,
                     gdiam_real z) {
  pnt[0] = x;
  pnt[1] = y;
  pnt[2] = z;
...
public:
  ...
  const gdiam_real &min_coord(int coord) const { return min_coords[coord]; }
  const gdiam_real &max_coord(int coord) const { return max_coords[coord]; }
  ...
  gdiam_real getLongestEdge() const { return length_dim(getLongestDim()); }
  ...
  gdiam_real get_min_coord(int dim) const { return min_coords[dim]; }
  gdiam_real get_max_coord(int dim) const { return max_coords[dim]; }
};
...
public:
  ...
  void combine(gdiam_point out, double a_coef, double b_coef, double c_coef) {
    for (int ind = 0; ind < GDIAM_DIM; ind++)
      out[ind] =
          a_coef * dir_1[ind] + b_coef * dir_2[ind] + c_coef * dir_3[ind];
  }
