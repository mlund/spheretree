...
public:
  ...
  const gdiam_point getCenter() const { return (gdiam_point)center; }
  ...
  gdiam_real maxDiam() const { return bbox_diam; }
  gdiam_real maxDiamProj() const { return bbox_diam_proj; }

  GFSPTreeNode *get_left() { return left; }
  GFSPTreeNode *get_right() { return right; }

  int size() const { return p_pnt_right - p_pnt_left + 1; }

  bool isSplitable() const { return (size() > 1); }

  const gdiam_point *ptr_pnt_left() { return p_pnt_left; }
  const gdiam_point *ptr_pnt_right() { return p_pnt_right; }

  friend class GFSPTree;

  const GBBox &getBBox() const { return bbox; }
};
...
public:
  ...
  const gdiam_point *getPoints() const { return arr; }

  int nodes_number() const { return root->nodes_number(); }
  ...
  gdiam_real brute_diameter(const gdiam_point *a_lo, const gdiam_point *a_hi,
                            const gdiam_point *b_lo, const gdiam_point *b_hi,
                            GPointPair &diam) const {
    double max_dist;
  ...
  const gdiam_point getPoint(int ind) const { return arr[ind]; }

  GFSPTreeNode *getRoot() { return root; }
...
public:
  ...
  void init(GFSPTreeNode *_left, GFSPTreeNode *_right, gdiam_point proj_dir,
            gdiam_real dist) {
    left = _left;
    right = _right;
    ...
    max_diam =
        max(max(bbox_proj_dist(_left->getBBox(), _right->getBBox(), proj_dir),
  ...
  double directDiameter(const GFSPTree &tree, GPointPair &diam) const {
    return tree.brute_diameter(left->ptr_pnt_left(), left->ptr_pnt_right(),
                               right->ptr_pnt_left(), right->ptr_pnt_right(),
                               diam);
  }
  ...
  double maxDiam() const { return max_diam; }
...
public:
  ...
  int size() const { return points_num; }
  const GPointPair &getDiameter() const { return pair_diam; }

  int nodes_number() const { return tree.nodes_number(); }
  ...
  void split_pair_proj(GFSPPair &pair, g_heap_pairs_p &heap, double eps,
                       gdiam_point proj);
  void compute_by_heap(double eps);
  void compute_by_heap_proj(double eps, gdiam_point proj);

  double diameter() { return pair_diam.distance; }
};
...
void *heap_top(heap_t *pHeap) { return pHeap->pArr[0]; }
...
public:
  g_heap_pairs_p() { heap_init(&heap, compare_pairs); }
  ...
  int size() const { return heap.curr_size; }
  GFSPPair top() { return *(GFSPPair *)heap_top(&heap); }
  void pop() {
...
void GTreeDiamAlg::addPairHeap(g_heap_pairs_p &heap, GFSPTreeNode *left,
                               GFSPTreeNode *right, gdiam_point proj,
                               GFSPPair &father) {
  const gdiam_point p(*(left->ptr_pnt_left()));
  const gdiam_point q(*(right->ptr_pnt_left()));
...
gdiam_bbox gdiam_approx_const_mvbb(gdiam_point *start, int size, gdiam_real eps,
                                   GBBox *p_ap_bbox) {
  // gdiam_real  diam;
  ...
  if ((pnt_length(dir) == 0.0) || (pnt_length(dir_2) < 1e-6) ||
...
public:
 ...
 point and slope */
  void compute_crossing(int a_ind, double a_angle, int b_ind, double b_angle,
                        gdiam_real &x, gdiam_real &y) {
  ...
  void get_bbox(int a_ind, gdiam_real a_angle, int b_ind, gdiam_real b_angle,
                int c_ind, gdiam_real c_angle, int d_ind, gdiam_real d_angle,
                bbox_2d_info &bbox, gdiam_real &area) {
    /*
    printf( "get_bbox: (%d, %g,\n"
            "%d, %g\n"
            "%d, %g\n"
    ...
    compute_crossing(a_ind, a_angle, b_ind, b_angle, bbox.vertices[0][0],
                     bbox.vertices[0][1]);
    compute_crossing(b_ind, b_angle, c_ind, c_angle, bbox.vertices[1][0],
                     bbox.vertices[1][1]);
    compute_crossing(c_ind, c_angle, d_ind, d_angle, bbox.vertices[2][0],
                     bbox.vertices[2][1]);
    compute_crossing(d_ind, d_angle, a_ind, a_angle, bbox.vertices[3][0],
                     bbox.vertices[3][1]);
  ...
  void get_angles(int ind, double &angle_1, double &angle_2, double &angle_3) {
    double angle;
...
public:
  ...
  void compute() {
    ...
    if ((!(fabs(pnt_dot_prod(base_proj, out_1)) < 1e-6)) ||
      ...
      printf("should be all close to zero: %g, %g, %g\n",
             pnt_dot_prod(base_proj, out_1), pnt_dot_prod(base_proj, out_2),
  ...
  gdiam_bbox &get_bbox() { return bbox; }
...
gdiam_bbox gdiam_mvbb_optimize(gdiam_point *start, int size, gdiam_bbox bb_out,
                               int times) {
  gdiam_bbox bb_tmp;
...
static void try_direction(gdiam_bbox &bb, gdiam_bbox &bb_out,
                          gdiam_point *start, int size, int x_coef, int y_coef,
                          int z_coef) {
  gdiam_bbox bb_new;
...
static void register_point(gdiam_point pnt, gdiam_point *tops,
                           gdiam_point *bottoms, int grid_size,
                           gdiam_bbox &bb) {
...
gdiam_point *gdiam_convex_sample(gdiam_point *start, int size, gdiam_bbox &bb,
                                 int sample_size) {
