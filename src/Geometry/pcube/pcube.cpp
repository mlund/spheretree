...
EXTERN int segment_intersects_cube(const real v0[3], const real v1[3]) {
  ...
  FOR(i, 3) {
    if (v0[i] * edgevec_signs[i] > .5)
      return 0;
    if (v1[i] * edgevec_signs[i] < -.5)
      return 0;
  }
  ...
  FOR(i, 3) {
    ...
    rhomb_normal_dot_v0 =
        edgevec[iplus2] * v0[iplus1] - edgevec[iplus1] * v0[iplus2];

    rhomb_normal_dot_cubedge = .5 * (edgevec[iplus2] * edgevec_signs[iplus1] +
